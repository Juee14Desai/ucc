                                          ...
                                          void *user_data)
{
    ...
    if (ucc_unlikely(UCS_OK != status)) {
        tl_error(UCC_TASK_LIB(task), "failure in rsv ring completion %s",
...
void ucc_tl_ucp_reduce_scatterv_ring_progress(ucc_coll_task_t *coll_task)
{
    ...
    ucc_rank_t              prevblock, recv_data_block;
    ucc_status_t            status;
    size_t                  max_block_size, block_offset, block_count;
    int                     step, is_avg, id;
    ...
    while (task->tagged.recv_posted > 0) {
        ...
        block_count  = ucc_coll_args_get_count(args,
                                               args->dst.info_v.counts,
                                               prevblock);
        block_offset = get_block_offset(args, prevblock);

        if (task->tagged.recv_completed == tsize - 1) {
            if (UCC_IS_INPLACE(*args)) {
                reduce_target = PTR_OFFSET(args->dst.info_v.buffer,
                                           get_block_offset(args, rrank) *
                                               dt_size);
            } else {
                reduce_target = args->dst.info_v.buffer;
            }
        }
        ...
        if (UCC_OK !=
            (status = ucc_dt_reduce(
                 r_scratch,
                 PTR_OFFSET(sbuf, block_offset * dt_size),
        ...
        recv_data_block = ucc_ring_pattern_get_recv_block(ring, 0, rrank,
                                                          (ucc_rank_t)step);
        block_count = ucc_coll_args_get_count(args,
                                              args->dst.info_v.counts,
                                              recv_data_block);
...
ucc_tl_ucp_reduce_scatterv_ring_start(ucc_coll_task_t *coll_task)
{
    ...
    recv_block  = ucc_ring_pattern_get_recv_block(ring, 0, rrank, 0);
    block_count = ucc_coll_args_get_count(args, args->dst.info_v.counts,
                                          recv_block);
    ...
    block_count  = ucc_coll_args_get_count(args, args->dst.info_v.counts,
                                           send_block);
    block_offset = get_block_offset(args, send_block);
    UCPCHECK_GOTO(ucc_tl_ucp_send_nb(PTR_OFFSET(sbuf,
                                                  block_offset * dt_size),
...
ucc_tl_ucp_reduce_scatterv_ring_finalize(ucc_coll_task_t *coll_task)
{
    ...
    ucc_mc_free(task->reduce_scatterv_ring.scratch_mc_header);
    return ucc_tl_ucp_coll_finalize(coll_task);
}

ucc_status_t ucc_tl_ucp_reduce_scatterv_ring_init_common(
    ucc_tl_ucp_task_t *task)
{
    ...
    dt              = args->dst.info_v.datatype;
    dt_size         = ucc_dt_size(dt);
    mem_type        = args->dst.info_v.mem_type;
    max_block_count = ucc_coll_args_get_max_count(args,
                                                   args->dst.info_v.counts,
                                                   tsize);

    /* 3 blocks: r_scratch (recv), s_scratch[0], s_scratch[1] (double-buffered send) */
    status = ucc_mc_alloc(&scratch_mc_header,
                          max_block_count * 3 * dt_size, mem_type);
    if (ucc_unlikely(UCC_OK != status)) {
        tl_error(UCC_TASK_LIB(task),
                 "failed to allocate scratch for reduce_scatterv ring");
        return status;
    }

    task->reduce_scatterv_ring.scratch           = scratch_mc_header->addr;
    task->reduce_scatterv_ring.scratch_mc_header = scratch_mc_header;
                                     ...
                                     ucc_coll_task_t     **task_h)
{
    ...
    status = ucc_tl_ucp_reduce_scatterv_ring_init_common(task);
    if (status != UCC_OK) {
